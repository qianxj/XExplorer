...
void FilePath::List(FilePathSet &directories, FilePathSet &files) {
	...
	HANDLE hFind = ::FindFirstFileA(wildCard.AsFileSystem(), &findFileData);
...
bool MakeLongPath(const char* shortPath, char* longPath) {
	...
	if (pfnGetLong != NULL) {
		...
		*longPath = '\0';
		...
		lstrcpynA(short_path, shortPath, _MAX_PATH);
		...
		for (;;) {
			...
			for (;;) {
				...
				hfind = ::FindFirstFileA(longPath, &fd);
				if (hfind == INVALID_HANDLE_VALUE)
					break;
	...
	if (!ok) {
		lstrcpynA(longPath, shortPath, _MAX_PATH);
	}
	return ok;
}
#endif
...
void FilePath::FixName() {
	...
	if (MakeLongPath(AsFileSystem(), longPath)) {
		...
		WIN32_FIND_DATAA FindFileData;
		HANDLE hFind = ::FindFirstFileA(AsFileSystem(), &FindFileData);
