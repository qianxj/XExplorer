...
namespace Hxsoft
{
	namespace XFrame
	{
		...
		//
		BOOL XSEdit::Create (LPCTSTR lpszWindowName, DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID)
		{
		...
		*/
		...
		LPSTR XSEdit::GetText ()
		{
			...
			if (lLen > 0)
			{
				...
				{
					*pReturn = '\0';
					SendMessage(this->m_hWnd,SCI_GETTEXT, lLen, (long)pReturn);
					return pReturn;
				}
			}
			return NULL;
			...
			LPCTSTR szPath) //@parm filename of to load
		{
			...
			int i;
			for(i=::_tcslen(szPath) -1;i>=0 && szPath[i]!='.';i--);
			if(i>=0)
			...
			LPCTSTR szPath) //@parm filename to save to
		{
			...
			if (pBuffer != NULL)
			{
				SendMessage(this->m_hWnd,SCI_GETTEXT, buflen,(long)pBuffer);
			...
			LPCTSTR szExtension) //@parm filename extension without dot e.g. "cpp"
		{
			...
			::_stprintf(buf,_T("|%s|\0"),szExtension);
		...
		int XSEdit::GetLinenumberWidth ()
		{
			...
			LRESULT lWidth = SendMessage(this->m_hWnd,SCI_TEXTWIDTH, STYLE_LINENUMBER, (long)_T("9"));
		...
		{
			SendMessage(this->m_hWnd,SCI_STYLESETFONT, nStyle, (long)szFontname);
		}
		/////////////////////////////////////
...
/*			SendMessage(this->m_hWnd,SCI_SETPROPERTY, (WPARAM)_T("fold"), (LPARAM)_T("1"));
			...
			LPSTR szText) //@parm text to search
		{
			...
			lPos = SendMessage(this->m_hWnd,SCI_FINDTEXT, m_nSearchflags, (long)&tf);
			if (lPos >= 0)
			{
				SetFocus(this->m_hWnd);
				GotoPosition(lPos);
				SendMessage(this->m_hWnd,SCI_SETSEL, tf.chrgText.cpMin, tf.chrgText.cpMax);
				SendMessage(this->m_hWnd,SCI_FINDTEXT, m_nSearchflags, (long)&tf);
				return TRUE;
			}
			return FALSE;
		}
			...
			LPCSTR szText) //@parm new text
		{
			...
			if (m_nSearchflags & SCFIND_REGEXP)
				SendMessage(this->m_hWnd,SCI_REPLACETARGETRE, strlen(szText), (long)szText);
			else
				SendMessage(this->m_hWnd,SCI_REPLACETARGET, strlen(szText), (long)szText);
		}
		/////////////////////////////////////
		...
		LPTSTR XSEdit::GetSelectedText()
		{
			...
			if (lLen > 0)
			{
				...
				{
					*p = '\0';
					SendMessage(this->m_hWnd,SCI_GETSELTEXT, 0, (long)p);
					return p;
				}
			}
			return _T("");
			...
			BOOL bUseSelection)
		{
			...
			if (bUseSelection)
			{
				...
				long lPos = SendMessage(this->m_hWnd,SCI_SEARCHINTARGET, strlen(szFind), (long)szFind);
				...
				while (lPos < lEnd && lPos >= 0)
				{
					if (m_nSearchflags & SCFIND_REGEXP) // check for regular expression flag
						lLen = SendMessage(this->m_hWnd,SCI_REPLACETARGETRE, strlen(szReplace), (long)szReplace);
					else
						lLen = SendMessage(this->m_hWnd,SCI_REPLACETARGET, strlen(szReplace), (long)szReplace);
					...
					lPos = SendMessage(this->m_hWnd,SCI_SEARCHINTARGET, strlen(szFind), (long)szFind);
					nCount++;
				}
			}
			else
