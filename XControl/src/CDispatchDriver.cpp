...
#define LPLP void**
#define AfxThrowOleException(x)

HRESULT GetClassIDFromString(LPCTSTR lpsz, LPCLSID lpClsID)
{
	HRESULT hr;
	const std::wstring strClsID(lpsz);
	if (lpsz[0] == '{')
	{		
		hr = CLSIDFromString(const_cast<LPOLESTR>(strClsID.c_str()), lpClsID);
	}
	else
	{
		hr = CLSIDFromProgID(strClsID.c_str(), lpClsID);
	}
	return hr;
...
CDispatchDriver::CDispatchDriver(LPDISPATCH lpDispatch, BOOL bAutoRelease)
{
	m_lpDispatch = lpDispatch;
	m_bAutoRelease = bAutoRelease;
}

CDispatchDriver::CDispatchDriver(const CDispatchDriver& dispatchSrc)
{
...
const CDispatchDriver&
CDispatchDriver::operator=(const CDispatchDriver& dispatchSrc)
{
	if (this != &dispatchSrc)
	{
...
BOOL CDispatchDriver::CreateDispatch(REFCLSID clsid, COleException* pError)
{
	...
	lpUnknown->QueryInterface(IID_IDispatch,(void **)&m_lpDispatch);
	if (m_lpDispatch == NULL)
		goto Failed;
...
BOOL CDispatchDriver::CreateDispatch(LPCTSTR lpszProgID,
	COleException* pError)
{
	...
	CLSID clsid;
	SCODE sc = GetClassIDFromString(lpszProgID, &clsid);
	if (FAILED(sc))
	{
		//if (pError != NULL)
		//	pError->m_sc = sc;
...
void CDispatchDriver::AttachDispatch(LPDISPATCH lpDispatch,
	BOOL bAutoRelease)
{
...
void CDispatchDriver::ReleaseDispatch()
{
	if (m_lpDispatch != NULL)
	{
		if (m_bAutoRelease)
...
LPDISPATCH CDispatchDriver::DetachDispatch()
{
...
/////////////////////////////////////////////////////////////////////////////
// CDispatchDriver implementation
...
void CDispatchDriver::InvokeHelperV(DISPID dwDispID, WORD wFlags,
	VARTYPE vtRet, void* pvRet, const BYTE* pbParamInfo, va_list argList)
{
	...
	if (vtRet != VT_EMPTY)
	{
		...
		case VT_BSTR:
			...
			if(vaResult.bstrVal)
			{
				pvRet = new wchar_t[wcslen(vaResult.bstrVal) + 1];
				wcscpy_s((wchar_t*)pvRet,wcslen(vaResult.bstrVal) + 1,vaResult.bstrVal);
			}
...
void  CDispatchDriver::InvokeHelper(DISPID dwDispID, WORD wFlags,
...
void CDispatchDriver::GetProperty(DISPID dwDispID, VARTYPE vtProp,
	void* pvProp) const
{
	((CDispatchDriver*)this)->InvokeHelper(dwDispID,
		DISPATCH_PROPERTYGET, vtProp, pvProp, NULL);
}

void  CDispatchDriver::SetProperty(DISPID dwDispID, VARTYPE vtProp, ...)
{
