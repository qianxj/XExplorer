...
int  CWorkStart::MakeData(XOffice::XXmlContentSvr * pContentSvr )
{
...
int  CWorkStart::MakeDataEx(XOffice::XXmlContentSvr * pContentSvr )
{
	IXMLDOMElementPtr pSchemaElement;
	vector<XOffice::XXmlContentSvr::TTreeItem>* pTreeData = NULL;
	pContentSvr->GetTreeData(pTreeData);
	if(pTreeData)delete pTreeData;
	pTreeData = new vector<XOffice::XXmlContentSvr::TTreeItem>;
	XOffice::XXmlContentSvr::TTreeItem item;
	for(int i=0;i<pContentSvr->GetRowCount();i++)
	{
		IXMLDOMElement * pElement;
		...
		IXMLDOMNode * pItem;
		pElement->selectSingleNode(L"row",&pItem);
		if(pItem)
		{
			item.children = true;
			item.expanded = false;
...
{
public:
	CWorkStart * m_pWorkStart;
	int OnRButtonDown(int nRow,TEvent* pEvent)
	{
		LPTSTR pUrl= L"dev:xpage[ActiveOpDlg.xpage]";

		RECT bound={0,0,400,600};
		xfControl* pControl = m_pWorkStart->GetControl(L"maillist");
		bound.left = pControl->m_pxfNode->m_AreaRect.left + pEvent->mouse.point.x;
		bound.top = pControl->m_pxfNode->m_AreaRect.top + pEvent->mouse.point.x;;
		bound.right = bound.left + 1;
		bound.bottom = bound.top + 1;

		POINT* pt = (POINT *)&bound;
		::ClientToScreen(pControl->GetHWND(),pt);
		bound.left = pt->x;
		bound.top = pt->y;
		pt++;
		::ClientToScreen(pControl->GetHWND(),pt);
		...
		LPARAM lParam = NULL;
		IXFWin::OpenWindow(m_pWorkStart->m_pXPage->m_pStrUrl,pUrl,
			(IXFWin *)m_pWorkStart->GetInterface(),lParam,NULL,
					WS_EX_TOOLWINDOW|WS_EX_NOACTIVATE,WS_DLGFRAME|WS_POPUP,
					&bound);
		return 1;
	}
...
public:
	...
	int OnRButtonDown(int nRow,TEvent* pEvent)
	{
		...
		RECT bound={0,0,400,600};
		xfControl* pControl = m_pWorkStart->GetControl(L"maillist");
		bound.left = pEvent->mouse.point.x;
		bound.top  =  pEvent->mouse.point.y;
		bound.right = bound.left + 1;
		bound.bottom = bound.top + 1;
		...
		if(var.bstrVal)
		{
			LPARAM lParam = (LPARAM)_tcsdup(var.bstrVal);
			IXFWin::OpenWindow(m_pWorkStart->m_pXPage->m_pStrUrl,pUrl,
				(IXFWin *)m_pWorkStart->GetInterface(),lParam,NULL,
						WS_EX_TOOLWINDOW|WS_EX_NOACTIVATE,/*WS_DLGFRAME|WS_CAPTION|WS_MAXIMIZEBOX|*/WS_THICKFRAME|WS_POPUP,
						&bound);
		}
		return 1;
	}
	...
	int OnRowFoucsChanged(int nNewRow,int nOldRow)
	{
		...
		int Row = pSheetSvr->GetReportDataSvr()->m_pContentSvr->GetContentRow(nNewRow);
		IXMLDOMElement * pElement = pSheetSvr->GetReportDataSvr()->m_pContentSvr->GetRowElement(Row);
		...
		if(var.bstrVal)
		{
			...
			if( xfUriLoader::Load(strUrl,NULL,buf,pData,nLen,false)==1)
			{
				...
				*/
				...
				int npl = 0;
				if(pl)npl = ((ILayerSheet *)pl->GetInterface())->GetSheetIndex();
				m_btask = false;
				if(pl)
				{
								...
								if(uri && action)
								{
									...
									if(IXAServer::ExecXQuery(m_pWorkStart->m_pXPage->m_pStrUrl,L"[business.task.action]",arg.GetString(), x)==1)
									{
										...
										for(int i=0;i<nlen;i++)
										{
											...
											_variant_t vName,vCaption,vPreUrl;
											...
											{	
												...
												pElement->getAttribute(L"pre.url",&vPreUrl);
												if(vPreUrl.bstrVal)
													pEle->setAttribute(L"pre.url",vPreUrl);
									...
									if(IXAServer::ExecXQuery(m_pWorkStart->m_pXPage->m_pStrUrl,L"[business.taskflow]",arg.GetString(), x)==1)
									{
										...
										for(int i=0;i<nlen;i++)
										{
											...
											if(vCaption.bstrVal && vUrl.bstrVal)
											{
												...
												if(!m_pWorkStart->m_pParam)m_pWorkStart->m_pParam = new TaskParam;
												ZeroMemory(m_pWorkStart->m_pParam,sizeof(TaskParam));

												if(uri)_tcscpy_s(((TaskParam *)m_pWorkStart->m_pParam)->uri,255,uri);
												if(action)_tcscpy_s(((TaskParam *)m_pWorkStart->m_pParam)->action,80,action);
												if(id)_tcscpy_s(((TaskParam *)m_pWorkStart->m_pParam)->billid,60,id);
												if(no)_tcscpy_s(((TaskParam *)m_pWorkStart->m_pParam)->billno,60,no);
												if(caption)_tcscpy_s(((TaskParam *)m_pWorkStart->m_pParam)->caption,255,caption);
												if(env)_tcscpy_s(((TaskParam *)m_pWorkStart->m_pParam)->env,40,env);
												if(taskid)_tcscpy_s(((TaskParam *)m_pWorkStart->m_pParam)->mailid,60,taskid);
												if(pid)_tcscpy_s(((TaskParam *)m_pWorkStart->m_pParam)->taskid,60,pid);
												if(para)_tcscpy_s(((TaskParam *)m_pWorkStart->m_pParam)->para,60,para);
					...
					if(npl > 0)
					{
						if(((ILayerSheet *)pl->GetInterface())->GetSheetCount() > 1)
						{
							((ILayerSheet *)pl->GetInterface())->SelectSheet(1);
						}
					}
				}
				pAttach->Release();
					...
					if(!pControl->m_pOleControl->QueryInterface(IID_IWebBrowser2, (void**)&webBrowser2))
					{
						if (!webBrowser2->get_Document(&lpDispatch))
						{
							if (!lpDispatch->QueryInterface(IID_IHTMLDocument2, (void**)&htmlDoc2))
							{
								...
								if(m_pWorkStart->m_viewType == CWorkStart::VT_Text || ((!bstrHtml || !bstrHtml[0]) && bstrText))
								{
									IHTMLElement *pElement;
									htmlDoc2->get_body(&pElement);
									if(pElement)pElement->put_innerText(bstrText);
								}
								else
								{
	...
	int OnDoubleClicked(int nRow)
	{
		...
		if(Row >0)
		{
			//IXMLDOMElement * pElement = pSheetSvr->GetRowElement(nRow);

			LPCTSTR pMailID = pSheetSvr->GetItemString(Row,_T("id"));
			if(pMailID)
			{
				HCURSOR hCursor = LoadCursor(NULL,IDC_WAIT);
				hCursor = ::SetCursor(hCursor);

				TaskParam * pParam = new TaskParam;
				::memset(pParam,0,sizeof(TaskParam ));

				_tcscpy_s(pParam->mailid ,60,pMailID); 
...
CWorkStart::CWorkStart(void):m_viewType(VT_SafeHtml),m_pMailListon(NULL)
{
	
}

CWorkStart::~CWorkStart(void)
{
	if(m_pParam) delete (TaskParam *)m_pParam; 
}

int CWorkStart::OnXCommand(LPCTSTR	pStrID, class xfControl * pControl)
{
	...
	if(pStrID && _tcsstr(pStrID,_T("hx-action:")))
	{
		...
		int Row = pSheetSvr->GetRow();
		int nRow = pSheetSvr->GetReportDataSvr()->m_pContentSvr->GetContentRow(Row);
		if(nRow >0)
		{
			...
			_tcscpy_s(pParam->mailid ,60,pMailID); 
			_tcscpy_s(pParam->uri ,255,m_pMailListon->m_rdf); 
			_tcscpy_s(pParam->action ,80,pStrID); 

			if(pControl && pControl->m_pxfNode && pControl->m_pxfNode->m_pElement)
			{
				_variant_t vPreUrl;
				pControl->m_pxfNode->m_pElement->getAttribute(L"pre.url",&vPreUrl);
				if(vPreUrl.bstrVal)
				{
					IXFWin::OpenWindow(m_pXPage->m_pStrUrl,vPreUrl.bstrVal,(IXFWin *)GetInterface(),(LPARAM)pParam);
					hCursor = ::SetCursor(hCursor);
					DestroyCursor(hCursor);
					if(pParam->pData == 0) return 1;
				}
			}
	...
	if(pStrID && _tcsstr(pStrID,_T("FolderPostChanged:")))
	{
		LPCTSTR pStr = _tcschr(pStrID,':') + 1;
		if(m_pMailListon && _tcsicmp(pStr,_T("workflow"))!=0 ) 
		{
			XOffice::XReport::XReportSheet* pSheet;
			XOffice::XReport::XReportSheetSvr* pSheetSvr;
			pSheet = (XOffice::XReport::XReportSheet*)GetControl(_T("maillist"));
	...
	if(pStrID && _tcsstr(pStrID,_T("FolderChanged:")))
	{
		...
		if(_tcsicmp(pStr,_T("localfolder"))!=0 && _tcsicmp(pStr,_T("mail"))!=0)
		{
			...
			LPTSTR pStrUri;
			pStrUri = pSheetSvr->GetReportDataSvr()->m_pContentSvr->m_pUri;
			pSheetSvr->GetReportDataSvr()->m_pContentSvr->m_pUri = L"uri.hxsoft.com/xaserver/entity/mail/list";
			pSheetSvr->Retrieve((LPTSTR)pStr);
			pSheetSvr->GetReportDataSvr()->m_pContentSvr->m_pUri = pStrUri;
