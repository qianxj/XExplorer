...
xCodeSheet::xCodeSheet(void):/*m_cp(null),*/m_xcp(null),m_xsscp(null),m_xsmparser(null),beval(false),m_btpp(true),
	m_wpp(null)
{
...
bool OnCodeEditCharEx(void * pOwner,Hxsoft::XFrame::XSEdit *pEdit,char ch)
{
	...
	if(_tcsstr(title,_T(".js"))) return false;
	if(_tcsstr(title,_T(".vl"))) return false;
	if(_tcsstr(title,_T(".vm"))) return false;
	if(_tcsstr(title,_T(".vx"))) return false;
	if(_tcsstr(title,_T(".vd"))) return false;
	if(_tcsstr(title,_T(".vf"))) return false;
	if(_tcsstr(title,_T(".voc"))) return false;
	if(_tcsstr(title,_T(".vc"))) return false;

	if(!_tcsstr(title,_T(".tpp"))) return false;
...
int xCodeSheet::OnXCommand(LPCTSTR	pStrID, class xfControl * pControl)
{
	...
	if(pStrID && _tcsicmp(pStrID,_T("xmExec"))==0)
	{
		...
		else if(_tcsstr(title,_T(".xss")))
		{
			...
			SendMessage(pOutPut->m_hWnd,EM_SCROLL,SB_LINEDOWN,0);
		}else if(_tcsstr(title,_T(".vl")) || _tcsstr(title,_T(".vm")) ||  _tcsstr(title,_T(".vf")) ||
			_tcsstr(title,_T(".vx"))||/* _tcsstr(title,_T(".vd")) ||*/ _tcsstr(title,_T(".voc")) ||  _tcsstr(title,_T(".vc")))
		{
			BSTR pXml = NULL;
			XSEdit * pControl = (XSEdit *)GetControl(_T("codecontent"));
			pControl->GetContent(&pXml);
		...
		}else if(_tcsstr(title,_T(".vd")))
		{
			...
			CWord fn(runtime.find_method(&context,L"test",L"main"),0);
			if(fn.get_index())runtime.call_method(&context,fn);
			delete perr;

			buf[255];
			_stprintf_s(buf,80,L"执行完成: ");
	...
	else if(pStrID && _tcsicmp(pStrID,_T("xmEval"))==0)
	{
		...
		}else*/ if(_tcsstr(title,_T(".xss")))
		{
			...
			SendMessage(pOutPut->m_hWnd,EM_REPLACESEL,false,(LPARAM)buf);
			SendMessage(pOutPut->m_hWnd,EM_SCROLL,SB_LINEDOWN,0);
		}else if(_tcsstr(title,_T(".xsm")) || _tcsstr(title,_T(".tpp")))
		{
			if(m_xsmparser) delete m_xsmparser;
			...
			*/
			...
			_tcsstr(title,_T(".vx"))|| _tcsstr(title,_T(".voc")) ||  _tcsstr(title,_T(".vc")))
		{
			/*if(m_xsmparser) delete m_xsmparser;
