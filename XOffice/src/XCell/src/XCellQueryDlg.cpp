#include "StdAfx.h"
#include "..\include\XCellQueryDlg.hpp"
...
{
	XCellQueryDlg::XCellQueryDlg(void):m_bCanDestroy(true)
	{
		m_bCloseFlag = false;
		SetEventListons();
	}

	XCellQueryDlg::~XCellQueryDlg(void)
	{
	...
	bool XCellQueryDlg::SetEventListons()
	{
		this->AttachListon(WM_ACTIVATE,(FNEvent)&XCellQueryDlg::EvtActive);
		//this->AttachListon(_T("edittype"),CBN_SELENDOK,(FNEvent)&XCellQueryDlg::EvtEditTypeSelected);
		this->AttachListon(_T("rpt"),DWV_DOUBLECLICKED,(FNEvent)&XCellQueryDlg::EvtGridDBClick);
		this->AttachListon(_T("rpt"),DWV_KEYDOWN,(FNEvent)&XCellQueryDlg::EvtGridKeyDown);
		return true;
	}
	bool XCellQueryDlg::UnSetEventListons()
	{
		this->ClearAllListonsTo();
		return true;
	}

	bool XCellQueryDlg::EvtActive(TEvent* pEvent,LPARAM lParam)
	{
	...
	bool XCellQueryDlg::EvtGridKeyDown(TEvent * pEvent, LPARAM lParam)
	{
	...
	bool XCellQueryDlg::EvtGridDBClick(TEvent * pEvent, LPARAM lParam)
	{
		return OnOK();
	}
	bool XCellQueryDlg::OnOK()
	{
	...
	int XCellQueryDlg::OnXCommand(LPCTSTR	pStrID, Hxsoft::XFrame::xfControl * pControl)
	{
		if(!pStrID) return 0;
		...
		else if(_tcsicmp(pStrID,_T("xmQuery"))==0)
		{
			...
			if(pItem && pItem->pStrXml)
			{
				...
				if(pNode)
				{
					...
					{
						LPTSTR pItemStr = _tcsdup(var.bstrVal);
						if(pItemStr[0]=='=')
						{
							CXGridComplier x;
							if(x.SetSource(pItemStr, pSvr, 1, pSvr->GetRow(), pSvr->GetCol()))
						...
						::GetWindowText(GetControl(_T("querystr"))->m_hWnd,strItem,1024);
						TCHAR WKURL[512];
						::_stprintf_s(WKURL,512,_T("<args><arg name='__wkurl' value='%s'/><arg name='querytxt' value='%s'/></args>"),pItemStr, strItem);
	...
	int XCellQueryDlg::OnOpenQuery(LPTSTR pData)
	{
		CGridFrameSvr * pSvr = (CGridFrameSvr *)m_pParam;
		LPTSTR pStr = NULL;
		PStrItem pItem = pSvr->GetGridDataSvr()->m_pContentSvr->GetItemData(pSvr->GetCol(),pSvr->GetRow());
		if(pItem && pItem->pStrText)
			pStr = pItem->pStrText;//pSvr->m_pEditSvr->m_pEdit->GetText();
		if(!pData)
		{
	...
	int XCellQueryDlg::OnOpen()
	{
		Hxsoft::XFrame::xfControl * xcQuery = GetControl(_T("querystr"));
		m_PrevWndFunc = (WNDPROC)GetWindowLongPtr(xcQuery->GetHWND(),GWLP_WNDPROC);
		SetWindowLongPtr(xcQuery->GetHWND(),GWLP_WNDPROC,(long)&XCellQueryDlg::WindowProc);
	...
	bool XCellQueryDlg::GetClientRect(RECT *pRect)
	{
		::GetClientRect(m_hWnd,pRect);
		return true;
	}
	HWND XCellQueryDlg::GetHWND()
	{
		return m_hWnd;
	}
	LRESULT CALLBACK XCellQueryDlg::WindowProc(HWND hwnd,UINT uMsg,WPARAM wParam,LPARAM lParam)
	{
		XCellQueryDlg * pThis = ((XCellQueryDlg *)GetProp(hwnd,L"thisex"));
		switch(uMsg)
		{
			case WM_KEYDOWN:
				{
