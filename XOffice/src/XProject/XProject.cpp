...
static tagXClassRelInfo XClassRelInfo[] = {
	{	_T("xproject"),	XFrameWndClass,WS_CHILD | WS_VISIBLE,	0	}
};
...
bool WINAPI CreateInstance(LPTSTR pStrObject,xbObject * &pxbObject)
{
	...
	if(nIndex>=0)
	{
		if(::_tcsicmp(pStrObject,_T("xproject"))==0)
		{
			pxbObject = new XOffice::XProject::XProjectSheet();
		}else
			return false;
