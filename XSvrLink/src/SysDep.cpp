...
static int WINAPI SysCrtReportHook(int iType, char *pszMsg, int *piRetVal);
static void SysRunThreadExitHooks(SYS_THREAD ThreadID, int iMode);
static int WINAPI SysSetSocketsOptions(SYS_SOCKET SockFD);
static int WINAPI SysRecvLL(SYS_SOCKET SockFD, char *pszBuffer, int iBufferSize);
static int WINAPI SysSendLL(SYS_SOCKET SockFD, char const *pszBuffer, int iBufferSize);
static unsigned int WINAPI SysThreadRunner(void *pRunData);
static int WINAPI SysThreadSetup(SYS_THREAD ThreadID);
static int WINAPI SysThreadCleanup(SYS_THREAD ThreadID);
...
static unsigned int WINAPI SysStkCall(unsigned int (*pProc) (void *), void *pData);
...
int WINAPI SysShutdownLibrary(int iMode)
{
...
int WINAPI SysGetHostByAddr(SYS_INET_ADDR const &AddrInfo, char *pszFQDN, int iSize)
{
	...
	StrNCpy(pszFQDN, pHostEnt->h_name, iSize);
...
int WINAPI SysExec(char const *pszCommand, char const *const *pszArgs, int iWaitTimeout,
	    int iPriority, int *piExitStatus)
{
	...
	for (i = 1; pszArgs[i] != NULL; i++)
		sprintf(StrAppend(pszCmdLine), " \"%s\"", pszArgs[i]);
	...
	BOOL bProcessCreated = CreateProcessA(NULL, pszCmdLine, NULL, NULL, FALSE,
...
int WINAPI SysCreateTlsKey(SYS_TLSKEY &TlsKey, void (*pFreeProc) (void *))
{
	EnterCriticalSection(&csTLS);
...
int WINAPI SysLockFile(const char *pszFileName, char const *pszLockExt)
{
	...
	HANDLE hFile = CreateFileA(szLockFile, GENERIC_READ | GENERIC_WRITE,
...
int WINAPI SysUnlockFile(const char *pszFileName, char const *pszLockExt)
{
	char szLockFile[SYS_MAX_PATH] = "";
...
{
	HMODULE hModule = LoadLibraryA(pszFilePath);
...
int WINAPI SysCloseModule(SYS_HANDLE hModule)
{
...
{
	HANDLE hEventSource = RegisterEventSourceA(NULL, szServerName);
...
int WINAPI SysExistFile(const char *pszFilePath)
{
	DWORD dwAttr = GetFileAttributesA(pszFilePath);
...
int WINAPI SysExistDir(const char *pszDirPath)
{
	DWORD dwAttr = GetFileAttributesA(pszDirPath);
...
int WINAPI SysMemoryInfo(SYS_INT64 *pRamTotal, SYS_INT64 *pRamFree,
