...
namespace Hxsoft
{
	namespace XFrame
	{
		class XCONTROL_API XSEdit : public xfControl 
		{
		public:
		...
		public:
			int LoadContent (BSTR bstr,LPCTSTR ext=_T(".xml")){return SetContent(bstr,ext);}
			int SetContent (BSTR bstr,LPCTSTR ext=_T(".xml"));
			int GetContent (BSTR *bstr);
		public: 
			xbObject * GetInterface(); 
		public:
			static LRESULT PASCAL WndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam);
			static WNDPROC m_OldWinProc;
		...
		public:
			...
			virtual void GetText (LPSTR &strText);
			// @cmember Get Text from the Scintilla control
			...
			virtual int ReplaceAll(LPSTR szFind, LPSTR szReplace, BOOL bSelection = TRUE);
		...
		protected:
			...
			int  m_nSearchflags;
		public:
			int AddExtent(void * pOwner,XSEdit *pEdit, bool (* pfOnChar)(void * pOwner,XSEdit *pEdit,char ch));
			int AddExtent(void * pOwner,XSEdit *pEdit,
