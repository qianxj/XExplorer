...
{
	const int nxuCsArrayDelta = 4;
	template< typename A, typename D > class xuCsArrayIterator;
	template< typename A, typename D > class /*XUTIL_API*/ xuCsArray
	{
	public:
		struct DataEntry
		{
		...
		xuCsArray( A nMaxAccess,
			const D& rValue,
			int nDelta = nxuCsArrayDelta );
		xuCsArray( A nMaxAccess,
		...
		void                        CopyFrom( const xuCsArray& rArray,
	...
	protected:
		...
		friend class xuCsArrayIterator<A,D>;
	...
	void xuCsArray<A,D>::Reset( const D& rValue )
	{
	...
	template< typename A, typename D >
	void xuCsArray<A,D>::SetValue( A nPos, const D& rValue )
	{
	...
	template< typename A, typename D >
	const D& xuCsArray<A,D>::GetValue( A nPos ) const
	{
	...
	template< typename A, typename D >
	const D& xuCsArray<A,D>::GetValue( A nPos, int& nIndex, A& nEnd ) const
	{
		nIndex = Search( nPos);
	...
	template< typename A, typename D >
	const D& xuCsArray<A,D>::GetValue( A nPos, int& nIndex, A& nStart,
		A& nEnd ) const
	{
		nIndex = Search( nPos);
	...
	template< typename A, typename D >
	const D& xuCsArray<A,D>::GetNextValue( int& nIndex, A& nEnd ) const
	{
		if (nIndex < nCount)
			++nIndex;
	...
	template< typename A, typename D >
	const D& xuCsArray<A,D>::GetPrevValue( int& nIndex, A& nStart ) const
	{
		if (nIndex > 0)
			--nIndex;
		nStart = (nIndex > 0 ? pData[nIndex-1].nEnd + 1 : 0);
	...
	template< typename A, typename D >
	int xuCsArray<A,D>::GetEntryCount() const
	{
		return nCount;
	}
	...
	template< typename A, typename D >
	const typename xuCsArray<A,D>::DataEntry&
		xuCsArray<A,D>::GetDataEntry( int nIndex ) const
	{
		return pData[nIndex];
	}
	...
	public:
		...
		void                        Resync( A nPos );
		template< typename X >
		void                        Follow( const xuCsArrayIterator<A,X>& );
	...
	template< typename A, typename D >
	xuCsArrayIterator<A,D>::xuCsArrayIterator(
		const xuCsArray<A,D> & rArrayP, A nStart, A nEnd )
		: rArray( rArrayP )
	...
	template< typename A, typename D >
	void xuCsArrayIterator<A,D>::NewLimits( A nStart, A nEnd )
	{
		nIterStart = nStart;
		nIterEnd = nEnd;
	...
	template< typename A, typename D >
	A xuCsArrayIterator<A,D>::GetIterStart() const
	{
		return nIterStart;
	}
	...
	template< typename A, typename D >
	A xuCsArrayIterator<A,D>::GetIterEnd() const
	{
		return nIterEnd;
	}
	...
	template< typename A, typename D >
	bool xuCsArrayIterator<A,D>::operator++()
	{
	...
	template< typename A, typename D >
	A xuCsArrayIterator<A,D>::GetPos() const
	{
		return nCurrent;
	}
	...
	template< typename A, typename D >
	bool xuCsArrayIterator<A,D>::NextRange()
	{
		if (!operator bool())
			return false;
	...
	template< typename A, typename D >
	xuCsArrayIterator<A,D>::operator bool() const
	{
		return !bEnd;
	}
	...
	template< typename A, typename D >
	const D& xuCsArrayIterator<A,D>::operator*() const
	{
	...
	template< typename A, typename D >
	A xuCsArrayIterator<A,D>::GetRangeStart() const
	{
		if (nIndex == 0)
	...
	template< typename A, typename D >
	A xuCsArrayIterator<A,D>::GetRangeEnd() const
	{
	...
	template< typename A, typename D >
	void xuCsArrayIterator<A,D>::Resync( A nPos )
	{
		if (nPos < nIterStart)
			nPos = nIterStart;
	...
	template< typename A, typename D > class /*XUTIL_API*/ xuSummableCsArray : public xuCsArray<A,D>
	{
	public:
		xuSummableCsArray( A nMaxAccessP,
			const D& rValue,
			int nDeltaP = nxuCsArrayDelta )
			: xuCsArray<A,D>( nMaxAccessP,
			rValue, nDeltaP)
		{}
		xuSummableCsArray( A nMaxAccessP,
			const D* pDataArray, int nDataCount )
	...
	template< typename A, typename D > class /*XUTIL_API*/ xuBitMaskCsArray : public xuCsArray<A,D>
	{
	public:
		xuBitMaskCsArray( A nMaxAccessP,
			const D& rValue,
			int nDeltaP = nxuCsArrayDelta )
			: xuCsArray<A,D>( nMaxAccessP, rValue, nDeltaP)
		{}
		xuBitMaskCsArray( A nMaxAccessP,
			const D* pDataArray, int nDataCount )
			: xuCsArray<A,D>( nMaxAccessP,
	...
	private:
		//xuCsArrayIterator<A,D>  aIter1;
		//xuCsArrayIterator<A,S>  aIter2;
		xuCsArrayIterator<A,D>*  pIter1;
		xuCsArrayIterator<A,S>*  pIter2;
		const D&                        rBitMask;
		const D&                        rMaskedCompare;
